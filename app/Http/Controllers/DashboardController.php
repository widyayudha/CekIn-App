...
class DashboardController extends Controller
{
    ...
    {
        $wt = new Weather();
        $weatherData = $wt->getCurrentByCity($city);

        if ($weatherData->cod === 200) {
            $icon = $weatherData->weather[0]->icon;
            $weatherIcon = "https://openweathermap.org/img/wn/" . $icon . "@2x.png";
            return inertia('WeatherResult', compact('weatherData', 'weatherIcon', 'city'));
        } else {
            // Handle case when city not found or other error
            return inertia('WeatherResultNotFound', compact('city'));
        }
    }
}
